...
<script lang="ts">
  ...
  type AuroraBackgroundProps = WithChildren<{
  ...
  let {
    showRadialGradient = $bindable(true),
    children,
    class: className,
    ...rest
...
<main class="h-full">
  <div
    class={cn(
      'transition-bg relative flex h-full flex-col items-center justify-center bg-zinc-50 dark:bg-zinc-900',
      className || ''
    )}
    {...rest}
  >
    <div
      class="absolute inset-0 overflow-hidden h-full"
      style="--aurora: repeating-linear-gradient(100deg,#3b82f6 10%,#a5b4fc 15%,#93c5fd 20%,#ddd6fe 25%,#60a5fa 30%);
